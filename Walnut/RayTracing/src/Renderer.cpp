...
void Renderer::Render(const Scene& scene, const Camera& camera)
{
	...
	//en este for each podemos decirle que se ejecute en paralelo
...
glm::vec4 Renderer::PerPixel(uint32_t x, uint32_t y)
{
	...
	for (int i = 0; i < bounces; i++)
	{
		...
		{
			glm::vec3 skyColor = glm::vec3(0.0f, 0.05f, 0.25f);
		...
		contribution *= material.Albedo; //1.0f - 0.0f
